...
import ru.vsu.edu.shlyikov_d_g.Element;
...
public class JTableUtils {
    ...
    public static void REinitJTableForArray(
            JTable table, int defaultColWidth,
            boolean showRowsIndexes, boolean showColsIndexes,
            boolean changeRowsCountButtons, boolean changeColsCountButtons,
            int changeButtonsSize, int changeButtonsMargin
    ...
    private static void writeArrayToJTable(JTable table, Object array, String itemFormat) {
        ...
        for (int i = 0; i < len1; i++) {
            if (rank == 1) {
                tableModel.setValueAt(String.format(itemFormat, get(array, i)), 0, i);
            } else {
                Object line = get(array, i);
                if (line != null) {
                    if (line.getClass().isArray()) {
                        int lineLen = Array.getLength(line);
                        for (int j = 0; j < lineLen; j++) {
                            tableModel.setValueAt(String.format(itemFormat, get(line, j)), i, j);
                        }
                    } else {
                        tableModel.setValueAt(String.format(itemFormat, get(array, i)), 0, i);
                    }
    ...
    public static <T> T[] readArrayFromJTable(
        ...
        return readArrayFromJTable(table, null, null, true, null);
    }
    ...
    public static Element[][] readElementMatrixFromJTable(JTable table) throws ParseException {
        String[][] matrix = readStringMatrixFromJTable(table);
        Element[][] elem = new Element[matrix.length][matrix.length];
        for (int i = 0; i < matrix.length; i++) {
            for (int j = 0; j < matrix.length; j++) {
                elem[i][j] = new Element(Integer.parseInt(matrix[i][j].substring(0, matrix[i][j].indexOf('/') > 0 ? matrix[i][j].indexOf('/') : matrix[i][j].length())),
                        matrix[i][j].indexOf('/') > 0 ? Integer.parseInt(matrix[i][j].substring(matrix[i][j].indexOf('/') + 1)) : 1);
                System.out.println(elem[i][j].print());
            }
        }
        return elem;
    }
